    ...
    Object.keys(viewports).forEach((viewportName: string) => {
      describe(`${viewportName} viewport`, {
        ...
        it('renders a sidebar with top, bottom, and profile nav', () => {
          ...
          cy.get('.kong-ui-sidebar-nav').should('be.visible')

          cy.get('.kong-ui-sidebar-nav').find('ul.top-items').should('be.visible')
          cy.get('.kong-ui-sidebar-nav').find('ul.top-items').find('li a').should('be.visible')
          cy.get('.kong-ui-sidebar-nav').find('ul.bottom-items').should('be.visible')
          cy.get('.kong-ui-sidebar-nav').find('ul.bottom-items').find('li a').should('be.visible')
        ...
        it('renders a router-link if item.to is an object', () => {
          ...
          cy.get('.kong-ui-sidebar-nav').find('.sidebar-item-link').should('have.class', 'router-link')
        })
        ...
        it('renders an anchor tag if item.to is a string that starts with `http`', () => {
          ...
          cy.get('.kong-ui-sidebar-nav').find('.sidebar-item-link').should('not.have.class', 'router-link')
        })
        ...
        it('renders an anchor tag and sets target="_blank" if item.newWindow is true, item.external is true, and item.to is a string', () => {
          ...
          cy.get('.kong-ui-sidebar-nav').find('.sidebar-item-link').should('not.have.class', 'router-link')
          cy.get('.kong-ui-sidebar-nav').find('.sidebar-item-link').should('have.attr', 'target', '_blank')
        })
        ...
        it('only renders a single anchor tag if item.newWindow is true', () => {
          ...
          cy.get('.kong-ui-sidebar-nav').find('.sidebar-item-link').should('not.have.class', 'router-link')
          cy.get('.kong-ui-sidebar-nav').find('.sidebar-item-primary a').should('have.length', 1)
        })
        ...
        describe('top items', () => {
          it('renders top nav items', () => {
            ...
            cy.get('.kong-ui-sidebar-nav').find('ul.top-items').should('be.visible')
            cy.get('.kong-ui-sidebar-nav').find('ul.top-items').find('li').should('be.visible')
          })
        })
        ...
        describe('bottom items', () => {
          it('renders bottom nav items', () => {
            ...
            cy.get('.kong-ui-sidebar-nav').find('ul.bottom-items').should('be.visible')
            cy.get('.kong-ui-sidebar-nav').find('ul.bottom-items').find('li').should('be.visible')
          })
        })
        ...
        describe('profile items', () => {
          it('does not render the profile item if no profileItems or profileName are passed', () => {
            ...
            cy.get('.kong-ui-sidebar-nav').find('.sidebar-footer').should('not.exist')
          })
          ...
          it('renders just the profileName if no nav items are passed', () => {
            ...
            cy.get('.kong-ui-sidebar-nav').find('.sidebar-profile-name').should('contain.text', name)
        ...
        it('displays a badge count if present for L2 items when the parent item is expanded', () => {
          ...
          cy.get('.kong-ui-sidebar-nav').find('.level-primary.top-items').find('li').eq(0).should('be.visible')
          cy.get('.kong-ui-sidebar-nav').find('.level-primary.top-items').find('.level-secondary').should('be.visible')
        ...
        it('renders an icon next to L1 items when provided', () => {
          ...
          cy.get('.kong-ui-sidebar-nav').find('.level-primary.top-items').find('li').eq(0).should('be.visible')
          cy.get('.kong-ui-sidebar-nav').find('.level-primary.top-items').find('.sidebar-item-icon .kong-icon').should('be.visible')
        })
        ...
        it('does not render an icon next to L1 items when not provided', () => {
          ...
          cy.get('.kong-ui-sidebar-nav').find('.level-primary.top-items').find('li').eq(0).should('be.visible')
          cy.get('.kong-ui-sidebar-nav').find('.level-primary.top-items').find('.sidebar-item-icon .kong-icon').should('not.exist')
        })
        ...
        describe('expanded', () => {
          ...
          it('displays a label under the L1 when provided and expanded', () => {
            ...
            cy.get('.kong-ui-sidebar-nav').find('.level-primary.top-items').find('li').eq(0).should('be.visible')
            cy.get('.kong-ui-sidebar-nav').find('.level-primary.top-items').find('.sidebar-item-label').should('contain.text', labelText)
          })
          ...
          it('does not display a label under the L1 when the item is not expanded', () => {
            ...
            cy.get('.kong-ui-sidebar-nav').find('.level-primary.top-items').find('li').eq(0).should('be.visible')
            cy.get('.kong-ui-sidebar-nav').find('.level-primary.top-items').find('.sidebar-item-label').should('not.exist')
          })
        })
        ...
        describe('events', () => {
          it('emits a `click` event with an `item` payload when clicked', () => {
            ...
            cy.get('.kong-ui-sidebar-nav').find('.sidebar-item-link').eq(1).click().then(() => {
  ...
  describe('mobile-only', () => {
    describe('window events', {
      ...
      it('closes the mobile menu when the window is resized', () => {
        ...
        cy.get('.kong-ui-sidebar-nav').should('have.class', 'sidebar-open')
        cy.get('.kong-ui-sidebar-nav').find('.level-primary.top-items li').should('be.visible')

        // Resize (downsize) the window viewport
        cy.viewport(viewports.mobile.width - 100, viewports.mobile.height)

        cy.get('.kong-ui-sidebar-nav').should('not.have.class', 'sidebar-open')
        cy.get('.kong-ui-sidebar-nav').find('.level-primary.top-items li').should('not.be.visible')
        cy.get('.kong-ui-sidebar-nav').should('not.be.visible')
      })
    })
    ...
    describe('features', () => {
      ...
      it('does not hide the sidebar off-screen when mobileEnabled is false', {
        ...
        cy.get('.kong-ui-sidebar-nav').should('be.visible')
        cy.get('.kong-ui-sidebar-overlay').should('not.exist')
      })
      ...
      it('hides the header slot when mobileHeaderVisible prop is false', () => {
        ...
        cy.get('.kong-ui-sidebar-nav').find('.sidebar-header').should('contain.text', headerSlotText)
        cy.get('.kong-ui-sidebar-nav').find('.sidebar-header').should('not.be.visible')
      })
      ...
      it('shows the overlay when the mobileOverlay prop is set to true', {
        ...
        cy.get('.kong-ui-sidebar-nav').should('be.visible')
        cy.get('.kong-ui-sidebar-overlay').should('be.visible')
      })
      ...
      it('closes the sidebar when a user clicks on the overlay', {
        ...
        cy.get('.kong-ui-sidebar-nav').should('be.visible')
        cy.get('.kong-ui-sidebar-overlay').click()
        cy.get('.kong-ui-sidebar-nav').should('not.be.visible')
      })
      ...
      it('sets the sidebar zIndex based on the zIndex prop value', {
        ...
        cy.get('.kong-ui-sidebar-nav').should('be.visible')
        cy.get('.kong-ui-sidebar-nav').should('have.css', 'z-index', String(zIndex))
      })
      ...
      it('sets the sidebar overlay zIndex based on the mobileOverlayZIndex prop value', {
        ...
        cy.get('.kong-ui-sidebar-nav').should('be.visible')
        cy.get('.kong-ui-sidebar-overlay').should('be.visible')
        cy.get('.kong-ui-sidebar-overlay').should('have.css', 'z-index', String(mobileOverlayZIndex))
      })
    })
  })
})
